...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($error)) {
        if (empty($name) || empty($email) || empty($mobile_number) || empty($metric_number) || empty($department_id)) {
            ...
            $add_stmt = $conn->prepare("INSERT INTO student (name, student_email, mobile_number, metric_number, department_id) VALUES (?, ?, ?, ?, ?)");
            $add_stmt->bind_param("ssssi", $name, $email, $mobile_number, $metric_number, $department_id);
            if ($add_stmt->execute()) {

                header("Location: index.php?route=students&status=add_success");
            } else {
                $error_message="$add_stmt->error";
            }
            $add_stmt->close();
        }
    } else {
...
<body>
    ...
    <select id="department_id" name="department_id" required>
        ...
        $dept_result = $conn->query("SELECT department_id, name FROM department");
